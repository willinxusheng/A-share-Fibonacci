...
_A_SHARE_MAKEUP = {  # 周末补班（仍交易）
    ...
    # TODO(每年末): 2027 补班日待国务院公布调休后补全（如春节前/国庆后调休补班；
    # 缺失会使该补班日被误当休市，影响 _fut 推演时间轴与 _gap_td 新鲜度护栏的微小精度）。
}
...
def forecast_svg(klines, r, wcls, conf, sigma, sym, horizon=60, bt=None, bt_paths=None, breadth_score=None):
    ...
    # 覆盖修正 κ：样本外黄金检验(逐时点滚动近窗口假装预测,5指数共6205点)实测——
...
def data_quality_strip(data, results):
    ...
    for sym, d in data.items():
        ...
        # 交叉验证未实际运行（第二数据源不可达 / 样本不足）→ 数据实为单源，必须提示而非假装通过
        if max_rel is None or n_cc < 3:
            ok_consist = False
            cons_txt = "未校验·单源"
        else:
            ok_consist = max_rel < 0.02
            cons_txt = "%.2f%%" % (max_rel * 100)
...
<style>
  * {{ box-sizing: border-box; margin: 0; padding: 0; }}
  ...
  @media (orientation: landscape) and (max-height: 560px) {{
    ...
    .echart-main {{ height: calc(100vh - 150px) !important; max-height: 380px; min-height: 200px; }}

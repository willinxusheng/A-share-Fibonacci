...
def sx(d):
    dd = ts(d) if isinstance(d, str) else d
    return mL + (dd - x0).total_seconds() / (x1 - x0).total_seconds() * (W - mL - mR)
...
for p in D["subForecast"]["points"]:
    proj.append((sx(p["date"]), sy(p["price"])))
proj_pts = " ".join(f"{x:.1f},{y:.1f}" for x, y in proj)

# x ticks monthly
labels = []
d = datetime(2026, 5, 1)
while d <= datetime(2026, 11, 1):
    labels.append((d.strftime("%Y-%m-%d"), d.strftime("%m月")))
    month = d.month + 1
    year = d.year
    if month > 12:
        month -= 12
...
for p in D["subForecast"]["points"]:
    ...
    if p.get("side") == "buy":
        symbols += f'<polygon points="{x},{y - 8} {x - 7},{y + 5} {x + 7},{y + 5}" fill="{color}"/>'
    elif p.get("side") == "sell":
        symbols += f'<polygon points="{x},{y + 8} {x - 7},{y - 5} {x + 7},{y - 5}" fill="{color}"/>'
    else:
        symbols += f'<circle cx="{x}" cy="{y}" r="5" fill="{color}"/>'
    pos = p.get("pos", "top")
    ly = y - 14 if pos == "top" else y + 20
    symbols += f'<text x="{x}" y="{ly}" font-size="11" fill="{color}" text-anchor="middle" font-weight="bold">{label}</text>'

# stop line
stop_price = D["tradePlan"]["stopLine"]["price"]
stop_y = sy(stop_price)

# 派生子浪ⅴ日期（避免 SVG 文案硬编码漂值）
_v5 = next((p for p in D["subForecast"]["points"] if "子浪ⅴ" in p.get("label", "")), None)
_v5d = _v5["date"] if _v5 else "—"
stop_line = f'<line x1="{mL}" y1="{stop_y:.1f}" x2="{W - mR}" y2="{stop_y:.1f}" stroke="#e6a23c" stroke-width="1.5"/>'
stop_line += f'<text x="{mL + 5}" y="{stop_y - 6:.1f}" font-size="11" fill="#e6a23c">铁律线 {stop_price}</text>'

# sell target horizontal line
s1p = D["tradePlan"]["sellTargets"][0]["price"]
s1y = sy(s1p)
s1_line = f'<line x1="{mL}" y1="{s1y:.1f}" x2="{W - mR}" y2="{s1y:.1f}" stroke="#c23531" stroke-width="1" stroke-dasharray="6,4"/>'
...
<svg xmlns="http://www.w3.org/2000/svg" width="{W}" height="{H}" viewBox="0 0 {W} {H}">
  ...
  {symbols}
  <text x="{W / 2}" y="25" font-size="16" fill="#16213e" text-anchor="middle" font-weight="bold">浪⑤子浪走势预测（修复后）</text>
  <text x="{W - 10}" y="{H - 10}" font-size="11" fill="#999" text-anchor="end">x轴 2026-05 至 2026-11-30；子浪ⅴ = {_v5d}</text>
</svg>'''
open("subf_fixed.svg", "w", encoding="utf-8").write(svg)
print("wrote subf_fixed.svg")
